if GAMEMODE_NAME == "terrortown" then 
  for _, wep in pairs( weapons.GetList() ) do
    if wep.ClassName == "fas2_m67" then
      ...
      SWEP.EquipMenuData = {
        type = "Weapon",
        desc = "M67 Fragmentation Grenade"
      };
      
      local enabled = GetConVar("ttt_fas2_wep_m67_enabled")
      local aquire = GetConVar("ttt_fas2_wep_m67_aquire")
      local available = GetConVar("ttt_fas2_wep_m67_available")
      local defaultfor = GetConVar("ttt_fas2_wep_m67_defaultfor")
      ...
      if enabled:GetInt() == 1 then 
        ...
        SWEP.AmmoEnt = "ttt_fas2_ammo_m67"
        SWEP.Icon = "VGUI/entities/fas2_m67"
        if SERVER then
          resource.AddFile( "materials/VGUI/entities/fas2_m67.vmt" )
        end 
      ...
      if SERVER then
        AddCSLuaFile("weapon_ttt_fas2_m67.lua")
      end
      ...
      hook.Add( "DoPlayerDeath", "DropCooked", function( ply )
        local wep = ply:GetActiveWeapon()
        if !wep.FuseTime then wep.FuseTime = ( CurTime() + 3.5 ) end
        if wep:GetClass() == "weapon_ttt_fas2_m67" then
          if wep.ThrowInitiated then
            local nade = ents.Create("fas2_thrown_m67")
            pos = ply:GetPos()
            ...
            if wep.ThrowType == "cook" then
              nade:Fuse(wep.FuseTime - CurTime())
            else
              nade:Fuse(3)
            end
          end
